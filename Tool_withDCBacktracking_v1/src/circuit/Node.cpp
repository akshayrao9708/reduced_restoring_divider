...
namespace vp
{
	...
	Node::Node(string name, Circuit* circuit, int revLevel, int nIndex)
	{
		this->name = name;
		this->circuit = circuit;
		tRise = tFall = 0;
	...
	void Node::assignTask(const NodeType typeOfNode, const string& portsDescription, const string& trise, const string& tfall)
	{
		...
		string sp = "[\\t\\n\\r ]*";
		string variableName("[a-zA-Z0-9\\[\\]_\\.\\\\]+");
		...
		if (regex_search(portsDescription, m, pattern1)){
			...
			if (!circuit->edges.count(gateName)) throw ParseError("wire \"" + gateName + "\" was not declared!(1)");
			...
			while (regex_search(remainingList, n, variableRegex)){
				if (!circuit->edges.count(n[1].str())) throw ParseError("wire \"" + n[1].str() + "\" was not declared!(2)");
	...
	{
		return (type == Node::INPUT_PORT);
	}

	bool Node::isOutputPort() const
	{
		return (type == Node::OUTPUT_PORT);
	}
	...
	{
		if (index >= getInputsCount()) throw std::out_of_range("(Node input) Node index out of range");
		return circuit->node(inputs[index]->getSource());
	}

	Node& Node::outputNode(size_t index)
	{
		if (index >= getOutputsCount()) throw std::out_of_range("(Node output) Node index out of range");
		size_t count = 0;
		...
		throw std::out_of_range("(Node output throw) Node index out of range");
	}
